...
export class PlatformAdminService {
  ...
  async login(dto: PlatformAdminLoginDto) {
    ...
    const adminTotp =
      process.env.PLATFORM_ADMIN_TOTP;
  ...
  async createSociety(
    ...
    const result =
      await this.prisma.$transaction(
        async (tx) => {
          ...
          const onboarding =
            await tx.societyOnboarding.create(
              {
                data: {
                  societyId:
                    society.id,
    ...
    return {
      ...
      chairman: {
        ...
        phone:
          result.chairman.phone,

        email:
          result.chairman.email,

        role:
          result.chairman.role,
      },
  ...
  async impersonate(
    ...
    if (!impersonatedUser) {
      throw new NotFoundException(
        'No active user found in society for impersonation',
      );
    }

    const primaryMapping =
      impersonatedUser.residentFlats.find(
        (mapping) =>
          mapping.isPrimary,
      ) ??
      impersonatedUser
